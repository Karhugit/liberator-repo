...
current_skin_prop, current_font_prop = 'liberator.current_skin', 'liberator.current_font'
addon_skins_folder = 'special://home/addons/plugin.video.liberator/resources/skins/Default/1080i/'
...
def window_manager(obj):
	def close():
		obj.close()
		clear_property('liberator.window_loaded')
		clear_property('liberator.window_stack')

	def monitor():
		timer = 0
		while not get_property('liberator.window_loaded') == 'true' and timer <= 5:
			sleep(50)
			timer += 0.05
		hide_busy_dialog()
		obj.close()
	...
	def get_stack():
		try: window_stack = json.loads(get_property('liberator.window_stack'))
		except: window_stack = []
		return window_stack

	def add_to_stack(params):
		window_stack.append(params)
		set_property('liberator.window_stack', json.dumps(window_stack))
...
class BaseDialog(window_xml_dialog):
	...
	def set_home_property(self, prop, value):
		set_property('liberator.%s' % prop, value)

	def get_home_property(self, prop):
		return get_property('liberator.%s' % prop)

	def clear_home_property(self, prop):
		return clear_property('liberator.%s' % prop)
...
class FontUtils:
	...
	def match_font(self, size, bold, fallback):
		font_tag = 'LIBERATOR_%s%s' % (size, '_BOLD' if bold else '')
