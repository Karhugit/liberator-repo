...
from indexers import dialogs, people, orac_lists
from indexers.images import Images
from modules import kodi_utils, settings, watched_status
from modules.sources import Sources
from modules.utils import change_image_resolution, adjust_premiered_date, get_datetime, batch_replace
...
random_choice, playback_choice, favorites_choice = dialogs.random_choice, dialogs.playback_choice, dialogs.favorites_choice
...
setting_base, label_base, ratings_icon_base = 'liberator.extras.%s.button', 'button%s.label', 'liberator_flags/ratings/%s'
separator = '[B]  •  [/B]'
...
class Extras(BaseDialog):
	...
	def onAction(self, action):
		...
		if action == self.info_action:
			...
			tmdb_id = chosen_listitem.getProperty('tmdb_id')
			ipc_params = {'tmdb_id': tmdb_id, 'item_type': self.media_type, 'user': settings.get_setting('trakt.user', '')}
			mode = 'get_movie_details' if self.media_type == 'movie' else 'get_show_details'
...
#		function = movie_meta if self.media_type == 'movie' else tvshow_meta
		...
		make_thread_list_enumerate(builder, data)
	...
	def make_reviews(self):
		...
		try:
			self.all_reviews = self.meta_get('all_reviews') or []
	...
	def make_comments(self):
		# Comments panel not yet implemented — stub to prevent AttributeError
	...
	def show_extrainfo(self, meta=None):
		if meta:
			text = separator.join([i for i in (meta.get('year'), str(round(meta.get('rating'), 1)) if meta.get('rating') not in (0, 0.0, None) else None,
									meta.get('certification'))]) + '[CR][CR]%s' % meta.get('overview')
			poster = meta.get('poster_path', empty_poster)
	...
	def assign_buttons(self):
		...
		for item in button_ids[:-1]:
			...
			try:
				...
				self.restore_setting_default({'setting_id': setting_id.replace('liberator.', ''), 'silent': 'true'})
				button_action = self.get_setting(setting_id)
				button_label = extras_button_label_values[self.media_type][button_action]
