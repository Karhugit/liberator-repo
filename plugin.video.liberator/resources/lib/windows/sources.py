...
'pm_cloud': get_icon('premiumize'), 'oc_cloud': get_icon('offcloud'), 'tb_cloud': get_icon('torbox'), 'orac': get_icon('orac')}
...
class SourcesResults(BaseDialog):
    def __init__(self, *args, **kwargs):
        ...
        self.scrape_duration = kwargs.get('scrape_duration', 0.0)
    ...
    def onAction(self, action):
        ...
        if action == self.info_action:
            ...
            if choice:
                ...
                if isinstance(choice, tuple) and choice[0] == 'mark_undesirable':
                    from apis.orac_api import _get_data_via_ipc
                    from modules.kodi_utils import notification, sleep
                    stream_name = choice[1]
                    _get_data_via_ipc('mark_undesirable', params={'stream_name': stream_name})
                    notification('Marked as Undesirable.', 2500)
                    sleep(250)
                    self.selected = ('rescrap', stream_name)
                    return self.close()
            ...
            for count, item in enumerate(results, 1):
                try:
                    ...
                    if scrape_provider == 'external':
                        ...
                        set_properties({'highlight': self.info_highlights_dict.get(key, ''), 'source_type': 'DIRECT', 'provider': upper(provider)})
    ...
    def set_properties(self):
        ...
        duration_str = ' (%.2fs)' % self.scrape_duration
        self.setProperty('filters_ignored', duration_str + (' | Filters Ignored' if self.filters_ignored else ''))
    ...
    def context_menu(self, item):
        ...
        choices_append(('Mark as Undesirable', ('mark_undesirable', name)))
...
class SourcesPlayback(BaseDialog):
    ...
    def set_scraper_properties(self):
        title, year = self.meta_get('title'), string(self.meta_get('year'))
        
        # Handle genre safely
        genre = self.meta_get('genres', [])  # Default to empty list instead of empty string
        if isinstance(genre, list):
            genre_string = ', '.join(genre)
        else:
            genre_string = str(genre) if genre else ''
    ...
    def set_resolver_properties(self):
        if self.meta_get('media_type') == 'movie': self.text = self.meta_get('plot') or self.meta_get('overview', '')
            ...
            self.text = '[B]%02dx%02d - %s[/B][CR][CR]%s' % (self.meta_get('season'), self.meta_get('episode'), self.meta_get('title', 'N/A').upper(), self.meta_get('overview', ''))
        self.setProperty('window_mode', self.window_mode)
