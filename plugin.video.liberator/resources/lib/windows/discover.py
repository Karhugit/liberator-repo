...
class Discover(BaseDialog):
	...
	def start_date(self):
		...
		if date_pattern.match(user_input):
			...
			if match:
				...
				return
		if final_date:
			url_insert_key = 'url_insert_movie' if self.media_type == 'movie' else 'url_insert_tvshow'
			self.set_key_values(self.chosen_item[url_insert_key] % final_date, final_date)
	...
	def make_url(self, active_attributes):
		...
		for key in active_attributes:
			url_part = self.get_attribute(self, key)
			offset_match = re.search(r'[Tt]([+-])(\d+)', url_part)
			if offset_match:
				try:
					operator = offset_match.group(1)
					days = int(offset_match.group(2))
					if operator == '+': dynamic_date = (datetime.now() + timedelta(days=days)).strftime('%Y-%m-%d')
					else: dynamic_date = (datetime.now() - timedelta(days=days)).strftime('%Y-%m-%d')
					offset_str_in_url = re.search(r'[Tt][+-]\d+', url_part).group(0)
					url_part = url_part.replace(offset_str_in_url, dynamic_date)
				except: pass
			url_parts.append(url_part)
		self.url = base_url % (('movie' if self.media_type == 'movie' else 'tv'), ''.join(url_parts))
