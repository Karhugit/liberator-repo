...
'ipc_data_db': ('CREATE TABLE IF NOT EXISTS requests (uuid TEXT PRIMARY KEY, data_json TEXT, timestamp REAL)',)
        }
media_prop = 'liberator.%s'
...
def clear_cache(cache_type, silent=False):
    ...
    if cache_type == 'meta':
        ...
        success = debrid_cache.clear_cache()
...
def get_ipc_db_path():
    """
    Returns the path to the IPC database.
    This is used for inter-process communication.
    """
    return ipc_data_db

def close_database(dbcon):
