...
def make_thread_list(_target, _list):
	max = min(len(_list),10)
	with ThreadPoolExecutor(max_workers=max) as executor:
	    executor.map(_target, _list)

def make_thread_list_multi_arg(_target, _list):
	max = min(len(_list),10)
	with ThreadPoolExecutor(max_workers=max) as executor:
	    executor.map(lambda args: _target(*args), _list)

def make_thread_list_enumerate(_target, _list):
	if not _list:
		return
	enumerated_list = [(index, item) for index, item in enumerate(_list)]
	max = min(len(enumerated_list),10)
	with ThreadPoolExecutor(max_workers=max) as executor:
		executor.map(lambda args: _target(*args), enumerated_list)
