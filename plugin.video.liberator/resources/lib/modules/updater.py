...
destination_check = translate_path('special://home/addons/plugin.video.liberator/')
changelog_location = translate_path('special://home/addons/plugin.video.liberator/resources/text/changelog.txt')
downloads_icon = get_icon('downloads')
addon_dir = 'plugin.video.liberator'
zipfile_name = 'plugin.video.liberator-%s.zip'
heading_str = 'Liberator Updater'
error_str = 'Error'
notification_occuring_str = 'Liberator Update Occuring'
notification_available_str = 'Liberator Update Available'
notification_updating_str = 'Liberator Performing Update'
notification_rollback_str = 'Liberator Performing Rollback'
...
success_str = '[CR]Success.[CR]Liberator updated to version [B]%s[/B]'
rollback_heading_str = 'Choose Rollback Version'
success_rollback_str = '[CR]Success.[CR]Liberator rolled back to version [B]%s[/B]'
confirm_rollback_str = 'Are you sure?[CR]Version [B]%s[/B] will overwrite your current installed version.' \
						'[CR]Liberator will set your update action to [B]OFF[/B] if rollback is successful'
...
no_changes_str = 'You are running the current version of Liberator.[CR][CR]There is no new version changelog to view.'

def get_location(insert=''):
	return 'https://github.com/%s/%s/raw/main/packages/%s' % (get_setting('liberator.update.username'), get_setting('update.location'), insert)

def get_versions():
	try:
		result = requests.get(get_location('liberator_version'))
...
def get_changes(online_version=None):
	try:
		...
		show_busy_dialog()
		result = requests.get(get_location('liberator_changes'))
		hide_busy_dialog()
...
def rollback_check():
	...
	results = [i['name'].split('-')[1].replace('.zip', '') for i in results.json() if 'plugin.video.liberator' in i['name'] \
