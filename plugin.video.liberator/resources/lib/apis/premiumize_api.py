...
class PremiumizeAPI:
	...
	def resolve_magnet_fast(self, magnet_url, info_hash, store_to_cloud, title, season, episode):
		try:
			file_url = None
			correct_files = []
			append = correct_files.append
			extensions = supported_video_extensions()
			if len(magnet_url) == 40: magnet_url = 'magnet:?xt=urn:btih:%s' % magnet_url
			...
			if season:
				...
				for item in valid_results:
					if seas_ep_filter(season, episode, item['path']):
						compare_link = seas_ep_filter(season, episode, item['path'], split=True)
			...
			return self.add_headers_to_url(file_url) if file_url else None
		except: return None

	def resolve_magnet(self, magnet_url, info_hash, store_to_cloud, title, season, episode, timeout=20):
		try:
			file_url = None
			correct_files = []
			append = correct_files.append
			...
			if len(magnet_url) == 40: magnet_url = 'magnet:?xt=urn:btih:%s' % magnet_url
			result = self.instant_transfer(magnet_url)
			logger('Premiumize', 'instant_transfer result: %s' % json.dumps(result))
			if not isinstance(result, dict) or not 'status' in result or result['status'] != 'success':
				logger('Premiumize', 'instant_transfer failed or invalid structure')
				return None
			valid_results = [i for i in result.get('content') if any(i.get('path').lower().endswith(x) for x in extensions) and not i.get('link', '') == '']
			logger('Premiumize', 'valid_results count: %s' % len(valid_results))
			if len(valid_results) == 0:
				logger('Premiumize', 'No valid results found in content')
				return None
			if season:
				episode_title = re.sub(r'[^A-Za-z0-9-]+', '.', title.replace('\'', '').replace('&', 'and').replace('%', '.percent')).lower()
				for item in valid_results:
					if seas_ep_filter(season, episode, item['path']):
						compare_link = seas_ep_filter(season, episode, item['path'], split=True)
						compare_link = re.sub(episode_title, '', compare_link)
						if any(x in compare_link for x in EXTRAS):
							logger('Premiumize', 'Filtering extra: %s (matches %s)' % (item['path'], [x for x in EXTRAS if x in compare_link]))
							continue
						file_url = item['link']
						break
				logger('Premiumize', 'Episode file_url: %s' % file_url)
			else:
				file_url = max(valid_results, key=lambda x: int(x.get('size'))).get('link', None)
				if file_url and not any(file_url.lower().endswith(x) for x in extensions): file_url = None
				logger('Premiumize', 'Movie file_url: %s' % file_url)
			if file_url:
				if store_to_cloud: Thread(target=self.create_transfer, args=(magnet_url,)).start()
				return self.add_headers_to_url(file_url)
		except Exception as e:
			logger('Premiumize', 'Exception in resolve_magnet: %s' % str(e))
			return None
