...
class TorBoxAPI:
	...
	def __init__(self):
		self.token = get_setting('liberator.tb.token')
	...
	def resolve_magnet(self, magnet_url, info_hash, store_to_cloud, title, season, episode, **kwargs):
		try:
			...
			selected_files = [{'url': '%d,%d' % (torrent_id, item['id']), 'filename': item['short_name'].replace('.lnk', ''), 'size': item['size']} \
							for item in torrent_files['data']['files'] if item['short_name'].lower().replace('.lnk', '').endswith(tuple(extensions))]
	...
	def display_magnet_pack(self, magnet_url, info_hash, **kwargs):
		...
		try:
			...
			torrent_id = torrent['data']['torrent_id']
			torrent_files = self.torrent_info(torrent_id)
			torrent_files = [{'link': '%d,%d' % (torrent_id, item['id']), 'filename': item['short_name'].replace('.lnk', ''), 'size': item['size']} \
							for item in torrent_files['data']['files'] if item['short_name'].lower().replace('.lnk', '').endswith(tuple(extensions))]
