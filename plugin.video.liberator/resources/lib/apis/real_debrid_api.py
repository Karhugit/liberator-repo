...
class RealDebridAPI:
    def __init__(self):
        self.client_ID = get_setting('liberator.rd.client_id', 'empty_setting')
        if self.client_ID in ('empty_setting', ''): self.client_ID = 'X245A4XAIBGVM'
        self.token = get_setting('liberator.rd.token', 'empty_setting')
        self.secret = get_setting('liberator.rd.secret', 'empty_setting')
        self.refresh = get_setting('liberator.rd.refresh', 'empty_setting')
        self.device_code = ''
    ...
    def resolve_magnet(self, magnet_url, info_hash, store_to_cloud, title, season, episode, timeout=20):
        ...
        try:
            ...
            if not torrent_info['links'] or 'error' in torrent_info:
                ...
                return None
            sleep(200)
            while elapsed_time <= timeout and not transfer_finished:
            ...
            if season:
                for value in selected_files:
                    if seas_ep_filter(season, episode, value[1]['path']):
                        compare_link = seas_ep_filter(season, episode, value[1]['path'], split=True)
                        compare_link = re.sub(compare_title, '', compare_link)
                        if any(x in compare_link for x in EXTRAS):
                            # logger('Real-Debrid', 'Filtering extra: %s' % value[1]['path'])
                            continue
                        match = True
                        index = [idx for idx, f in enumerate(torrent_info['files']) if f['path'] == value[1]['path']][0]
                        break
            else:
                ...
                for value in selected_files:
                    ...
                    match = True; index = [idx for idx, f in enumerate(torrent_info['files']) if f['path'] == value[1]['path']][0]; break
            if match:
