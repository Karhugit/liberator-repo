...
from modules.kodi_utils import make_session, clear_property, sleep
...
def import_easynews():
	...
	# if get_setting('liberator.easynews.api_version') == '0': return EasyNewsAPI()
...
class EasyNewsAPI:
	def __init__(self):
		...
		self.username = get_setting('liberator.easynews_user', 'empty_setting')
		self.password = get_setting('liberator.easynews_password', 'empty_setting')
	...
	def search(self, query):
		...
		return self._process_search(url)
	...
	def resolver(self, url_dl):
		...
		for i in range(3):
			try:
				headers = {'Authorization': self.auth}
				response = session.get(url_dl, headers=headers, stream=True, timeout=30.0)
				if response.status_code in (200, 206):
					resolved_link = response.url
					if resolved_link != url_dl:
						logger('easynews API resolver', f'Successfully resolved link on attempt {i+1}')
						break
				logger('easynews API resolver', f'Attempt {i+1} failed with status code: {response.status_code}')
				if i < 2: sleep(2000)
			except requests.exceptions.RequestException as e:
				logger('easynews API resolver', f'Attempt {i+1} failed with exception: {e}')
				if i < 2: sleep(2000)
