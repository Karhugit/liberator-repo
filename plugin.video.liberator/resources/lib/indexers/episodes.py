...
from modules.utils import jsondate_to_datetime, adjust_premiered_date, make_day, get_datetime, title_key, date_difference
...
from modules.watched_status import get_database, watched_info_episode, get_next
from modules.watched_status import get_watched_status_tvshow, watched_info_tvshow
string =  str
...
def build_episode_list(params):
	...
	watched_title = 'Trakt' if watched_indicators == 1 else 'Liberator'
...
#			episodes_data = episodes_meta(orig_season, meta)
	...
	if list_type == 'episode.next':
		...
		ipc_params = params.get('ipc_params', {'user': get_setting('trakt_user', '')})
		data = get_next_episodes(nextep_content, ipc_params=ipc_params)
		...
		else: resformat, resinsert, list_type = '%Y-%m-%d %H:%M:%S', '2000-01-01 00:00:00', 'episode.next_liberator'
	...
	# SXM Here is the data we use for next episodes
	make_thread_list_enumerate(_process, data)
	if return_results:
