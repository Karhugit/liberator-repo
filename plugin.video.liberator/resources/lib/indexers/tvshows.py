...
trakt_main = ('trakt_tv_most_watched',)
trakt_special = ()
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external:
				...
				if not any([x in folderpath for x in internal_nav_check]): set_property('liberator.exit_params', folderpath)
			...
			if self.action in main:
				...
				if self.params_get('get_imdb'):
					self.params['key_id'] = self.params_get('tmdb_id', self.params_get('key_id'), tmdb_api_key(), mpaa_region(), get_datetime(), get_current_timestamp())['imdb_id']
...
#			meta = tvshow_meta(self.id_type, _id, self.tmdb_api_key, self.mpaa_region, self.current_date, self.current_time)
			...
			if unaired: playcount, total_watched, total_unwatched = 0, 0, total_aired_eps
			else:
			...
			set_properties({'totalepisodes': string(total_aired_eps), 'totalseasons': string(total_seasons)})
			if self.is_external:
			...
			set_properties({'liberator.extras_params': extras_params, 'liberator.options_params': options_params, 'liberator.more_like_this_params': more_like_this_params})
	...
	def worker(self):
		...
		self.watched_title = 'Trakt' if self.watched_indicators == 1 else 'Liberator'
		...
		if self.custom_order:
			make_thread_list_multi_arg(self.build_tvshow_content, self.list)
		else:
			make_thread_list_enumerate(self.build_tvshow_content, self.list)
