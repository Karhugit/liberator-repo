...
def browse_tb_cloud(folder_id, media_type):
	...
	video_files = [{**i, 'media_type': media_type} for i in files['data']['files'] if i['short_name'].lower().replace('.lnk', '').endswith(tuple(extensions))]
	handle = int(sys.argv[1])
	add_items(handle, list(_builder()))
	set_content(handle, 'files')
	end_directory(handle)
...
def resolve_tb(params):
	...
	LiberatorPlayer().run(resolved_link, 'video')
