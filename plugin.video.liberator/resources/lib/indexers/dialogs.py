...
kodi_refresh, autoscrape_next_episode = kodi_utils.kodi_refresh, settings.autoscrape_next_episode
...
def playback_choice(params):
    logger('playback_choice', 'playback_choice')
    ...
    if meta:
        try:
            ...
            logger('playback_choice', 'Failed to decode meta JSON')
            return notification('Cancelled', 2500)
    if media_type == 'episode':
        poster = meta.get('show_poster_path') or poster_empty
        tmdb_id = meta['show_tmdb_id']
    else:
        poster = meta.get('poster_path') or poster_empty
    ...
    if choice == 'scrape':
        ...
        else: play_params = {'mode': 'playback.media', 'media_type': 'episode', 'tmdb_id': tmdb_id, 'season': season, 'episode': episode, 'autoplay': 'false'}
    elif choice == 'clear_and_rescrape':
        if media_type == 'movie': play_params = {'mode': 'playback.media', 'media_type': 'movie', 'tmdb_id': tmdb_id, 'autoplay': 'false'}
        else: play_params = {'mode': 'playback.media', 'media_type': 'episode', 'tmdb_id': tmdb_id, 'season': season, 'episode': episode, 'autoplay': 'false'}
    elif choice == 'scrape_with_default':
        if media_type == 'movie': play_params = {'mode': 'playback.media', 'media_type': 'movie', 'tmdb_id': tmdb_id,
                                                'default_ext_only': 'true', 'prescrape': 'false', 'autoplay': 'false'}
        else: play_params = {'mode': 'playback.media', 'media_type': 'episode', 'tmdb_id': tmdb_id, 'season': season,
        ...
        play_params = {'mode': 'playback.media', 'media_type': 'episode', 'tmdb_id': tmdb_id, 'season': season, 'episode': episode, 'prescrape': 'false',
        ...
        if media_type in ('movie', 'movies'): play_params = {'mode': 'playback.media', 'media_type': 'movie', 'tmdb_id': tmdb_id,
                        'custom_title': custom_title, 'prescrape': 'false'}
        else: play_params = {'mode': 'playback.media', 'media_type': 'episode', 'tmdb_id': tmdb_id, 'season': season, 'episode': episode,
                            'custom_title': custom_title, 'prescrape': 'false'}
    elif choice == 'scrape_with_custom_values':
        default_title, default_year = meta['title'], str(meta['year'])
        if media_type in ('movie', 'movies'): play_params = {'mode': 'playback.media', 'media_type': 'movie', 'tmdb_id': tmdb_id, 'prescrape': 'false'}
        else: play_params = {'mode': 'playback.media', 'media_type': 'episode', 'tmdb_id': tmdb_id, 'season': season, 'episode': episode, 'prescrape': 'false'}
        if aliases:
...
def set_quality_choice(params):
    ...
    fl = ['SD', '720p', '1080p', '4K']
    try: preselect = [fl.index(i) for i in get_setting('liberator.%s' % quality_setting).split(', ')]
    except: preselect = []
    ...
    if not orig_button_dict:
        for _type in ('movie', 'tvshow'):
            ...
            for item in range(10, 18):
                ...
                try:
                    button_action = get_setting('liberator.%s' % setting_id)
                    button_label = extras_button_label_values[_type][button_action]
                except:
                    set_setting(setting_id.replace('liberator.', ''), default_setting_values(setting_id.replace('liberator.', ''))['setting_default'])
                    button_action = get_setting('liberator.%s' % setting_id)
                    button_label = extras_button_label_values[_type][button_action]
...
def set_language_filter_choice(params):
    ...
    try: preselect = [fl.index(i) for i in get_setting('liberator.%s' % filter_setting_id).split(', ')]
    except: preselect = []
...
def enable_scrapers_choice(params={}):
    icon = params.get('icon', None) or get_icon('liberator')
...
def scraper_color_choice(params):
    ...
    current_setting, original_highlight = get_setting('liberator.%s' % setting), default_setting_values(setting)['setting_default']
...
def options_menu_choice(params, meta=None):
    ...
    if not meta:
        ...
        meta = _get_data_via_ipc(mode, params=ipc_params)
    meta_get = meta.get
    ...
    if not from_extras:
        ...
        if menu_type in ('tvshow', 'season', 'episode'): listing_append(('TV Shows Progress Manager', ''))
...
def _orac_meta_to_extras_meta(orac_meta, media_type):
    """
    Maps an Orac API /movie or /show response dict into the internal meta schema
    that the Extras window (extras.py) expects.
    """
    get = orac_meta.get
    # Runtime: Orac stores minutes as integer, Extras expects seconds (duration in seconds)
    runtime_mins = get('runtime') or 0
    try:
        duration_secs = int(float(runtime_mins)) * 60
    except (TypeError, ValueError):
        duration_secs = 0

    # extra_info: build a safe dict so Extras never crashes on .get()
    extra_info = get('extra_info') or {}
    if not isinstance(extra_info, dict):
        extra_info = {}
    # Ensure all keys Extras accesses are present
    extra_info.setdefault('status', get('status') or '')
    extra_info.setdefault('collection_id', (get('belongs_to_collection') or {}).get('id') if isinstance(get('belongs_to_collection'), dict) else None)
    extra_info.setdefault('collection_name', (get('belongs_to_collection') or {}).get('name', '') if isinstance(get('belongs_to_collection'), dict) else '')
    extra_info.setdefault('budget', get('budget') or '')
    extra_info.setdefault('revenue', get('revenue') or '')
    extra_info.setdefault('homepage', get('homepage') or '')
    extra_info.setdefault('last_episode_to_air', get('last_episode_to_air'))
    extra_info.setdefault('next_episode_to_air', get('next_episode_to_air'))
    extra_info.setdefault('type', get('type') or '')
    extra_info.setdefault('created_by', get('created_by') or '')

    title = get('title') or get('name') or ''
    year = get('year') or ''
    ...
    mapped = {
        ...
        'tmdb_id':          get('tmdb_id'),
        'imdb_id':          get('imdb_id'),
        'trakt_id':         get('trakt_id'),
        'title':            title,
        'year':             year,
        'rootname':         rootname,
        'plot':             get('plot') or get('overview') or '',
        'tvshow_plot':      get('tvshow_plot') or '',
        'tagline':          get('tagline') or '',
        'rating':           get('rating') or 0.0,
        'votes':            get('votes') or 0,
        'mpaa':             get('mpaa') or get('certification') or '',
        'duration':         duration_secs,
        'genre':            get('genre') or get('genres') or [],
        'studio':           get('studio') or [],
        'director':         get('director') or [],
        'writer':           get('writer') or [],
        'cast':             get('cast') or [],
        'spoken_language':  get('spoken_language') or get('language') or '',
        'premiered':        get('premiered') or get('released') or '',
        'poster':           get('poster') or get('poster_path') or '',
        'fanart':           get('fanart') or get('fanart_path') or '',
        'clearlogo':        get('clearlogo') or get('clearlogo_path') or '',
        'landscape':        get('landscape') or get('landscape_path') or '',
        'all_trailers':     get('all_trailers') or [],
        'trailer':          get('trailer') or '',
        'keywords':         get('keywords') or [],
        'total_seasons':    get('total_seasons') or 0,
        'total_aired_eps':  get('total_aired_eps') or 0,
        'season_data':      get('season_data') or [],
        'extra_info':       extra_info,
        'extra_ratings':    get('extra_ratings'),
    }
    return mapped
...
def extras_menu_choice(params):
    ...
    if meta:
        try:
            ...
            logger('extras_menu_choice', 'Failed to decode meta JSON')
            if not stacked: hide_busy_dialog()
            return notification('Cancelled', 2500)
    if not meta:
        # Fetch meta from Orac — same pattern as options_menu_choice
        ...
        if orac_meta and isinstance(orac_meta, dict):
            ...
            # Fetch TMDB reviews on-demand (only for the Extras window, covers both movies and TV shows)
            reviews_params = {'tmdb_id': tmdb_id, 'media_type': content, 'max_reviews': '20'}
            reviews_response = _get_data_via_ipc('get_reviews', params=reviews_params)
            if reviews_response and reviews_response.get('success'):
                meta['all_reviews'] = reviews_response.get('reviews', [])
    if not stacked: hide_busy_dialog()
    if not meta:
        return notification('Could not load metadata', 2500)
