...
from modules.utils import manual_function_import, paginate_list, get_current_timestamp, get_datetime
...
movie_main = ( 'tmdb_movies_blockbusters','tmdb_movies_in_theaters', 'tmdb_movies_upcoming', 'tmdb_movies_latest_releases')
movie_trakt_main = ('trakt_movies_trending', 'trakt_movies_trending_recent')
...
tvshow_main = ('tmdb_tv_popular_today', 'tmdb_tv_premieres', 'tmdb_tv_airing_today','tmdb_tv_on_the_air','tmdb_tv_upcoming',
'tmdb_anime_popular', 'tmdb_anime_popular_recent', 'tmdb_anime_premieres', 'tmdb_anime_upcoming', 'tmdb_anime_on_the_air')
tvshow_trakt_main = ('trakt_tv_most_watched',)
...
'tmdb_anime_year': meta_lists.years_tvshows, 'tmdb_anime_decade': meta_lists.decades_tvshows,
'tmdb_anime_genres': meta_lists.anime_genres, 'tmdb_anime_providers': meta_lists.watch_providers_tvshows}
tvshow_trakt_special = ()
memory_str = 'liberator.%s'
...
class RandomLists():
	def __init__(self, params):
		...
		self.remake_widgets = self.is_external and get_property('liberator.refresh_widgets') == 'true'
	...
	def random_main(self):
		...
		if not random_list:
			...
			make_thread_list(lambda x: self.random_results.extend(list_function(x)['results']), self.get_sample())
	...
	def random_special_main(self):
		...
		if not random_list:
			...
			if self.action in tvshow_trakt_special:
				make_thread_list(lambda x: self.random_results.extend(list_function(info['id'], x)), self.get_sample())
			else:
				make_thread_list(lambda x: self.random_results.extend(list_function(info['id'], x)['results']), self.get_sample())
...
#					meta_function = movie_meta if self.menu_type == 'movie' else tvshow_meta
	...
	def make_directory(self, next_page_params={}):
		...
		if self.is_external:
			if self.folder_name: set_property('liberator.%s' % self.folder_name, self.category_name)
			else: set_property('liberator.%s' % self.base_list_name, self.category_name)
...
def random_shortcut_folders(folder_name, random_results):
	...
	remake_widgets = is_external and get_property('liberator.refresh_widgets') == 'true'
	...
	if is_external: set_property('liberator.%s' % folder_name, list_name)
